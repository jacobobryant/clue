...
(def humans (reaction (:game/players @game)))
(def ais (reaction (:game/ais @game)))
(def players (reaction (concat @humans @ais)))
(def can-start-game? (reaction (<= 2 (count @players))))
(def can-add-ai? (reaction (<= (count @players) 5)))
(def can-remove-ai? (reaction (not= 0 (count @ais))))
...
(def available-locations (reaction (when @post-roll?
                                     (->> (core/available-locations @current-location @roll)
                                        ...
                                        (into #{})))))
(def observer? (reaction (contains? (:game/observers @game) @username)))
