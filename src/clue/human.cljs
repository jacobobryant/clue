...
(defn-spec current-board ::c/board
  ...
  (let [[player-coordinate player-room]
        ...
        (concat player-coordinate
                (mapcat player-coordinates-for (u/map-inverse player-room)))]
    (reduce (fn [board [character coordinate]]
              (assoc board coordinate (info/character-chars character)))
            c/empty-board
            player-coordinate)))
...
(defn-spec print-game-board any?
  [spaces ::c/board]
  (print "   ")
  (apply println (map #(char (+ (.charCodeAt "a" 0) %)) (range c/board-width)))
  (println
    (str/join
      "\n"
      (for [i (range (count c/board))]
        (gstring/format "%2d %s" (inc i)
                (str/join
                  " "
...
(defn print-rooms []
  (println "Rooms:"
           (->> c/room-chars
                (map-indexed #(gstring/format "(%s) %s" %1 (c/name-of %2)))
                (str/join ", "))))
